...
// 
// void  NFCNetClientModule::SendByServerID(const int nServerID, const uint16_t nMsgID, const char * strData , const uint32_t nLen)
// {
// 	std::ostringstream stream;
// 	stream << " NFCNetClientModule::SendByServerID " << nServerID;
// 	stream << " msg id " << nMsgID;
// 	m_pLogModule->LogError(stream, __FUNCTION__, __LINE__);
// 
// 	NF_SHARE_PTR<ConnectData> pServer = mxServerMap.GetElement(nServerID);
...
// 		if (pNetModule.get())
// 		{
// 			//if(pNetModule->GetNet()->SendMsg(strData, (size_t) nLen,0 ))
// 			if (!pNetModule->SendMsg(nMsgID, strData, 0))
// 			{
// 				std::ostringstream stream;
// 				stream << " SendMsgWithOutHead failed " << nServerID;
...
//	m_pLogModule->LogInfo("This is a client, begin to print Server Info-------------------");
    ...
    while (nullptr != pServerData)
    {
		...
		{
			if (m_LastSInfoLogTime + 60 > GetPluginManager()->GetNowTime())
			{
				m_LastSInfoLogTime = GetPluginManager()->GetNowTime();
				m_pLogModule->LogInfo(stream.str());
			}		
		}
		else
...
int NFCNetClientModule::OnDisConnected(const NFSOCK fd, NFINet* pNet)
{
    ...
    {
		std::ostringstream stream;
		stream << "DISCONNECT!!!!  Type: " << pServerInfo->eServerType << " Server ID: " << pServerInfo->nGameID << " State: "
			<< pServerInfo->eState << " IP: " << pServerInfo->strIP << " Port: " << pServerInfo->nPort;
...
void NFCNetClientModule::ProcessAddNetConnect()
{
    ...
    for (; it != mxTempNetList.end(); ++it)
    {
        ...
        if (nullptr == xServerData)
        {
			...
			xServerData->nGameType = xInfo.nGameType;
