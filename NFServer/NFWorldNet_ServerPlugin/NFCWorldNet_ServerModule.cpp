...
// 	
// 	int nodeDb = 100;
// 	std::string strIp = "192.168.9.53";
// 	int nPort = 18095;
// 
// 
// 	NF_SHARE_PTR<ServerData> pServerData = mGameMap.GetElement(nodeDb);
// 	if (!pServerData)
// 	{
// 		pServerData = NF_SHARE_PTR<ServerData>(NF_NEW ServerData());
// 		
// 
// 		pServerData->pData->set_server_id(nodeDb);
// 		pServerData->pData->set_server_name("node dbserver");
// 		pServerData->pData->set_server_cur_count(0);
// 		pServerData->pData->set_server_ip(strIp);
// 		pServerData->pData->set_server_port(nPort);
// 		pServerData->pData->set_server_max_online(1);
// 		pServerData->pData->set_server_state(NFMsg::EST_NARMAL);
// 		pServerData->pData->set_server_type(NF_SERVER_TYPES::NF_ST_DB);
// 		mGameMap.AddElement(nodeDb, pServerData);
// 	}
// 	std::ostringstream strLog;
// 	strLog << "try init connect node db server, GameID = " << std::to_string(nodeDb);
// 	strLog << "ip:" << strIp << "  port:" << std::to_string(nPort);
// 	m_pLogModule->LogNormal(NFILogModule::NLL_ERROR_NORMAL, NULL_OBJECT, strLog, __FUNCTION__, __LINE__);
    return true;
}
...
void NFCWorldNet_ServerModule::OnGameServerRegisteredProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    ...
    for (int i = 0; i < xMsg.server_list_size(); ++i)
    {
        ...
        pServerData->nFD = nSockIndex;
        *(pServerData->pData) = xData;
		m_pLogModule->LogInfo(std::to_string(xData.game_type()), __FUNCTION__, __LINE__);
        m_pLogModule->LogNormal(NFILogModule::NLL_INFO_NORMAL, NFGUID(0, xData.server_id()), xData.server_name(), "GameServerRegistered");
    }
