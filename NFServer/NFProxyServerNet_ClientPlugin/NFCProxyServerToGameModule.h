...
protected:
	...
	//ws game msg 向上的箭头 统一 Transpond 处理
	void OnLoginResp(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen);
	//t掉
	void OnClientOfflineResp(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen);
	//延迟处理关闭
	void OnDelayOfflineProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen);
private:
