...
// 	//×¢²á×Ô¼ºµÄdb 
// 	int nodeDbServerID = 100;
// 	std::string  nodeDbIp = "192.168.9.53";
// 	int  nodeDbPort = 18095;
// 	std::string nodeName = "node dbserver";
// 	int  nodeDbnMaxConnect = 1;
// 	NFMsg::ServerInfoReportList xnodeDbMsg;
// 	NFMsg::ServerInfoReport* pData = xnodeDbMsg.add_server_list();
// 
// 	pData->set_server_id(nodeDbServerID);
// 	pData->set_server_name(strName);
// 	pData->set_server_cur_count(0);
// 	pData->set_server_ip(nodeDbIp);
// 	pData->set_server_port(nodeDbPort);
// 	pData->set_server_max_online(nodeDbnMaxConnect);
// 	pData->set_server_state(NFMsg::EST_NARMAL);
// 	pData->set_server_type(NF_SERVER_TYPES::NF_ST_DB);
// 
// 	NF_SHARE_PTR<ConnectData> pServerData = m_pNetClientModule->GetServerNetInfo(pNet);
// 	if (pServerData)
// 	{
// 		int nTargetID = pServerData->nGameID;
// 		m_pNetClientModule->SendToServerByPB(nTargetID, NFMsg::EGameMsgID::EGMI_GTW_GAME_REGISTERED, xnodeDbMsg);
// 	}
// 	std::ostringstream strLog;
// 	strLog << "try init connect node db server, GameID = " << std::to_string(nodeDbServerID);
// 	strLog << "ip:" << nodeDbIp << "  port:" << std::to_string(nodeDbPort);
	...
	if (xLogicClass)
	{
		...
		for (int i = 0; i < strIdList.size(); ++i)
		{
			...
			if (pPluginManager->GetAppID() == nServerID)
			{
				...
				const std::string& strName = m_pElementModule->GetPropertyString(strId, NFrame::Server::Name());
				const std::string& strIP = m_pElementModule->GetPropertyString(strId, NFrame::Server::IP());
				int gameType = m_pElementModule->GetPropertyInt32(strId, NFrame::Server::GameType());
...
void NFCGameServerToWorldModule::OnServerInfoProcess(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	...
	for (int i = 0; i < xMsg.server_list_size(); ++i)
	{
		...
		xServerData.nGameType = xData.game_type();
	...
	//if (iDbconNum == 0)
	//{
	//	ConnectData xDbTempServerData;
	//	xDbTempServerData.nGameID = 100;
	////	xDbTempServerData.strIP = "192.168.9.53";
	//	xDbTempServerData.strIP = "127.0.0.1";

	//	xDbTempServerData.nPort = 18095;
	//	xDbTempServerData.strName = "node dbserver";
	//	xDbTempServerData.nWorkLoad = 1;
	//	xDbTempServerData.eServerType = NF_SERVER_TYPES::NF_ST_DB;
	//	if (m_pNetClientModule->GetServerNetInfo(NF_ST_DB) == NULL)
	//	{
	//		m_pNetClientModule->AddServer(xDbTempServerData);
	//		std::ostringstream strLog;
	//		strLog << "try connect node db server, GameID = " << xDbTempServerData.nGameID;
	//		strLog << "ip:" << xDbTempServerData.strIP << "  port:" << std::to_string(xDbTempServerData.nPort);
	//		m_pLogModule->LogNormal(NFILogModule::NLL_ERROR_NORMAL, NULL_OBJECT, strLog, __FUNCTION__, __LINE__);
	//	}
	//
	//
