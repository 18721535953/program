...
bool NFCGameServerNet_ServerModule::AfterInit()
{
	...
	//////////////////////////////////////////////////////////////////////////
	//数据转发处理
	m_pNetModule->AddReceiveCallBack(NFMsg::EGMI_PTG_PROXY_USER_DATA, this, &NFCGameServerNet_ServerModule::OnP2GProxyUserData);
	...
	if (xLogicClass)
	{
		...
		for (int i = 0; i < strIdList.size(); ++i)
		{
			...
			if (nServerType == NF_SERVER_TYPES::NF_ST_GAME && pPluginManager->GetAppID() == nServerID)
			{
				...
				m_GameType = m_pElementModule->GetPropertyInt32(strId, NFrame::Server::GameType());
...
int NFCGameServerNet_ServerModule::AddNetGameDispFunc(const int nMsgID, const NET_GAME_DESP_FUNC_PTR& cb)
{
	if (mxGameDispCallBack.find(nMsgID) == mxGameDispCallBack.end())
	{
		std::list<NET_GAME_DESP_FUNC_PTR> xList;
		xList.push_back(cb);
		mxGameDispCallBack.insert(std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::value_type(nMsgID, xList));
		return true;
	}

	std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::iterator it = mxGameDispCallBack.find(nMsgID);
	it->second.push_back(cb);
	return true;

}

int NFCGameServerNet_ServerModule::AddNetGameDispFunc(const int nGameType,const int nMsgID, const NET_GAME_DESP_FUNC_PTR& cb)
{
	if (mxGameTypeGameDispCallBack.find(nGameType) == mxGameTypeGameDispCallBack.end())
	{
		std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>> mxTempGameDispCallBack;

		std::list<NET_GAME_DESP_FUNC_PTR> xList;
		xList.push_back(cb);
		mxTempGameDispCallBack.insert(std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::value_type(nMsgID, xList));

		mxGameTypeGameDispCallBack.insert(std::map<const int, std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>>::value_type(nGameType, mxTempGameDispCallBack));
	...
	std::map<const int, std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>>::iterator ittype = mxGameTypeGameDispCallBack.find(nGameType);

	if (ittype->second.find(nMsgID) == ittype->second.end())
	{
		std::list<NET_GAME_DESP_FUNC_PTR> xList;
		xList.push_back(cb);
		ittype->second.insert(std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::value_type(nMsgID, xList));

		mxGameTypeGameDispCallBack.insert(std::map<const int, std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>>::value_type(nGameType, ittype->second));
		return true;
	}

	std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::iterator it = ittype->second.find(nMsgID);
	it->second.push_back(cb);
...
void NFCGameServerNet_ServerModule::OnP2GProxyUserData(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
	NFMsg::P2GProxyUserInfoDataPack xSkipPack;
	...
	if (NFINetModule::ReceivePB(nMsgID, msg, nLen, xSkipPack, nPlayer) == false)
	{
		stream << "error ------> 解析数据包 faile\n";
		stream << "socketIndex:" << nSockIndex << " ";
		stream << " msg id " << nMsgID;
		stream << " OnP2GProxyUserData TEST Data: " << msg;
	}
	else
	{
		const NFGUID& xClient = NFINetModule::PBToNF(xSkipPack.infoid());
		//NFGUID info = xx.infoid();
		stream << "client id :" << xClient.ToString();
		stream << " u id:" << xSkipPack.userid();
		stream<< "data :" << xSkipPack.data();
		
		nlohmann::json jsonData;
		// 
		try
		{
			jsonData = nlohmann::json::parse(xSkipPack.data());
			if (jsonData.find("action") == jsonData.end())
			{
				std::cout << "no find action\n";
				return;
			}
			else
			{
				msgid = jsonData.at("action");
				std::cout << " find \n";
			}

			//if (jsonData.find("GameType") == jsonData.end())
			//{
			//	m_pLogModule->LogError("没有 GameType", __FUNCTION__, __LINE__);
			//	gameType = m_GameType;
			//}
			//else
			//{
			//	gameType = jsonData.at("GameType").get<int>();
			//}
		}
		...
		if (OnNetGameDispFunc(m_GameType, msgid, nSockIndex, nPlayer, jsonData))
		{
