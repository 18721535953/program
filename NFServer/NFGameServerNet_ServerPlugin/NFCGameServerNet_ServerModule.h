...
public:
	...
	virtual int GetGameType() { return m_GameType; }
...
protected:
	...
	//////////////////////////////////////////////////////////////////////////
	//用户数据
	void OnP2GProxyUserData(const NFSOCK nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen);
protected:
...
protected:
	...
	int AddNetGameDispFunc(const int nMsgID, const NET_GAME_DESP_FUNC_PTR& cb);
	int AddNetGameDispFunc(const int nGameType, const int nMsgID, const NET_GAME_DESP_FUNC_PTR& cb);
	inline bool OnNetGameDispFunc(const int nMsgID, const NFSOCK fd,const NFGUID clientID, const nlohmann::json& jsonObject)
	{
		std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::iterator it = mxGameDispCallBack.find(nMsgID);
		if (it == mxGameDispCallBack.end()) return false;

		for (auto functionIter : it->second)
		{
			functionIter->operator()(fd, clientID, jsonObject);
		}
		return true;
	}
	inline bool OnNetGameDispFunc(const int nGameType,const int nMsgID, const NFSOCK fd, const NFGUID clientID, const nlohmann::json& jsonObject)
	{
		std::map<const int, std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>>::iterator it = mxGameTypeGameDispCallBack.find(nGameType);
		
		if (it == mxGameTypeGameDispCallBack.end())
		{
			std::cout << "OnNetGameDispFunc fail is game type is null \n";
			return false;
		}
		std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>::iterator it_msg = it->second.find(nMsgID);
		
		if (it_msg == it->second.end())
		{
			std::cout << "OnNetGameDisFunc fail is nMsgId null \n";
			return false;
		}

		for (auto functionIter : it_msg->second)
		{
			functionIter->operator()(fd, clientID, jsonObject);
		}
		return true;
...
private:
	...
	// GameDisp
	std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>> mxGameDispCallBack;

	std::map<const int , std::map<const int, std::list<NET_GAME_DESP_FUNC_PTR>>> mxGameTypeGameDispCallBack;
